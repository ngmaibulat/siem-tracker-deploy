...
| `meilisearch` | Поисковый движок, том `meili_data`, только внутренняя сеть. Полнотекстовый поиск по комментариям источников и feature requests (`MEILI_URL`/`MEILI_MASTER_KEY` на сервисе `app`); при недоступности приложение прозрачно откатывается на подстрочный поиск в БД. `MEILI_ENV=production` требует `MEILI_MASTER_KEY` |
