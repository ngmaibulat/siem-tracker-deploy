...
Linux prod
  ...
  ├─ docker compose up -d               # приложение само применяет миграции при старте
  └─ проверка приложения
```
...
# Необязательно: APP_URL и TARGET_DATE оба задаются Мастером первоначальной
# настройки (/setup) при первом запуске и сохраняются в БД/реестре — задавайте
# их здесь только если нужно зафиксировать стабильное значение между
# перезапусками (пустое значение переменной равносильно её отсутствию).
# TARGET_DATE=2026-06-15
# APP_URL=https://siem.example.local
# Необязательно: мастер-ключ Meilisearch. Сервис стартует с MEILI_ENV=development
# (только внутренняя сеть, порт не публикуется), поэтому работает и без ключа —
# без собственной аутентификации Meilisearch. Задайте, если нужна аутентификация
# запросов к Meilisearch. Сгенерировать: openssl rand -base64 32
...
| `TARGET_DATE` | (опц.) целевой срок прогноза — задаётся Мастером настройки; переменная нужна только чтобы зафиксировать значение |
| `APP_URL` | (опц.) публичный HTTPS-адрес (для ссылок в письмах) — за nginx это `https://<host>`; задаётся Мастером настройки, переменная нужна только чтобы зафиксировать значение |
| `MEILI_MASTER_KEY` | (опц.) мастер-ключ Meilisearch — сервис работает и без него (`MEILI_ENV=development`) |
...
| `meilisearch` | Поисковый движок, том `meili_data`, только внутренняя сеть. Полнотекстовый поиск по комментариям источников и feature requests (`MEILI_URL`/`MEILI_MASTER_KEY` на сервисе `app`); при недоступности приложение прозрачно откатывается на подстрочный поиск в БД. Стартует с `MEILI_ENV=development`, поэтому `MEILI_MASTER_KEY` необязателен |
...
Так как доступ теперь идет через nginx, адрес хоста в письмах со ссылками
берется из значения, сохраненного на шаге «App URL» мастера настройки — вводите
его там **с HTTPS**, например `https://siem.example.local`. Переменная `APP_URL`
в `.env` не обязательна: задавайте ее только если нужно зафиксировать значение
поверх того, что сохранено мастером, например `APP_URL=https://siem.example.local`.
...
Миграции схемы применяются самим приложением при каждом запуске (FR-47) — оно
мигрирует **оба плана**, ориентируясь на подключения, реально разрешенные
SQLite-реестром (а не на `DATABASE_URL`/`CONTROL_DATABASE_URL` — так что
переключение активного подключения в реестре подхватывается тем же
механизмом, без отдельного шага под конкретную БД), защищено advisory
lock'ом от параллельного старта нескольких контейнеров и никогда не роняет
запуск сам по себе — реальный gate остается за проверкой обязательных таблиц
сразу после. Отключается через `AUTO_MIGRATE=off` (тогда приложение лишь
предупреждает в логе о неприменённых миграциях). Отдельного сервиса/шага
`migrate` в этом репозитории больше нет.
Сида админа тоже нет: на чистой установке (нет учётных записей) приложение
запускает **Мастер первоначальной настройки** (`/setup`), где создаётся первый
администратор и задаются APP_URL / SMTP / целевая дата / TLS — схема при этом
создаётся самим мастером, независимо от механизма выше.
...
Достаточно одной команды `docker compose up -d app` — приложение само
применит все ожидающие миграции при старте, до того как начнет отвечать на
запросы.
...
> 5. Запустить приложение (`docker compose up -d app`) — миграции применятся автоматически при старте.
>
...
4. docker compose up -d            # поднять весь стек (app мигрирует себя сам при старте)
```
...
7. Будет поднят стек (docker compose up -d) — приложение само применит
   ожидающие миграции при старте.
```
