...
- **Domain plane (MariaDB, by wizard default)**: `mariadb-master` + `mariadb-slave1`/`mariadb-slave2`, classic async binlog/GTID replication. The app's `DB_MARIADB_URL` points only at the master — no read/write splitting. Which plane actually ends up on MariaDB at runtime is a choice made once in the Initial Configuration Wizard (stored in the connection registry, never re-derived from env at runtime — see the app repo's CLAUDE.md, Connection registry section); the app then migrates that registry-resolved target itself at every start (FR-47).
- **Control plane (Postgres, by wizard default)**: the `postgres` service hosts a base `siem_source_tracker` database (identity/auth lives in its own `control` schema within it once the wizard assigns it there) — it does not double as the domain-plane database, matching the intent that this compose file's default topology is domain=MariaDB / control=Postgres — but, as above, the wizard is what actually decides at runtime, and the app migrates whatever it chooses itself. It also doubles as the FR-42 restore-helper (`RESTORE_PG_URL`) for staging legacy pg_dump restores.
...
- **Scheduler (FR-46, scheduled report delivery)**: the `scheduler` service runs the `ngmaibulat/usiem-scheduler` image (versioned in lockstep with the app image) — a thin Bun trigger that polls the app's `/api/scheduler/config` and fires due jobs via `/api/scheduler/run`; all real work (rendering PDFs, sending mail) happens inside the app. `SCHEDULER_TOKEN` in `.env` is optional — these two routes are only reachable over the internal compose network (nginx blocks `/api/scheduler/*` on the public listener), so no token is required for a normal deployment; set one for defense-in-depth.
